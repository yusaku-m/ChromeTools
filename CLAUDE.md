...
`music_room` is also exempted from the normal sync window (`now - 1 day` .. `now + 365 days`, applied in `ScheduleSync.py`): `gcal_other` (all non-music_room calendars) and `gcal_music_room` are tracked separately through the parse loop, and only `gcal_other` gets `filtering_by_date()` applied before the input diff — `gcal_music_room` is diffed against the *unfiltered* `ccal` (`gcal_music_room.substract(ccal, ...)`) instead of `ccal_filter`, so a music_room event years in the past or future is still recognized as "already synced" and not endlessly re-input. The delete side is intentionally left unchanged (still computed from the normal windowed `gcal_filter`/`ccal_filter`, which includes music_room's in-window subset via the `gcal = gcal_other.union(gcal_music_room)` used there) — `ccal` has no per-event source-calendar tag, so widening the delete comparison to all time would risk flagging unrelated old Cybozu entries (from any calendar) that merely fall outside the window as deletable. For the unfiltered `ccal` comparison to actually see far-past auto-synced entries, `Cyboze.get_calender()` also best-effort-selects the earliest available `StartDate.Year`/`StartDate.Month` on the `PersonalScheduleExport` form (wrapped in try/except since those field names are guessed by symmetry with `EndDate.Year`/`EndDate.Month` and haven't been confirmed against the live form).
